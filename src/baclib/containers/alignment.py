...
from baclib.utils.protocols import HasIntervals
...
class ClusterBatch(RaggedBatch):
    ...
    def empty(self) -> 'ClusterBatch':
        # Override RaggedBatch.empty because __init__ signature differs
        return ClusterBatch(np.empty(0, dtype=object), np.array([0], dtype=np.int32))

    def __repr__(self): return f"<ClusterBatch: {len(self)} clusters>"
    ...
    def __getitem__(self, item):
        if isinstance(item, (int, np.integer)):
            ...
            return Cluster(nodes, id_=self._ids[item], score=self._scores[item], representative=self._representatives[item])
