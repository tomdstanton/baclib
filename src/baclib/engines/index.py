...
@SparseMapIndex.register_kernel(SparseMapIndexMode.DENSE)
def _setup_dense(index, **kwargs):
    """
    Dense indexing strategy: Stores EVERY k-mer (Stride=1).
    Essential for Motif Counting / Exhaustive Statistics.
    """
    # Kernel Args: bits, mask, dtype, rc_table
    return _dense_kernel, (index.bps, index.mask, index.dtype, index.rc_table)
...
@jit(nopython=True, cache=True, nogil=True, parallel=True)
def _dense_kernel(data, starts, lengths, k, bits, mask, dtype, rc_table):
    n_seqs = len(starts)
    counts = np.zeros(n_seqs, dtype=np.int32)

    # Pass 1: Count (It's just Length - K + 1)
    for i in prange(n_seqs):
        l = lengths[i]
        if l >= k:
            counts[i] = l - k + 1
    ...
    for i in prange(n_seqs):
        ...
        # Use a rolling hash approach manually for max speed
        rc_high_shift = bits * (k - 1)
        curr_f = dtype(0)
        curr_r = dtype(0)

        # Pre-load first k-mer
        for j in range(k):
            val = data[start + j]
        ...
        # Store first
        out_hashes[off] = curr_f if curr_f < curr_r else curr_r
        out_pos[off] = 0
        out_ids[off] = i
        ...
        for j in range(1, l - k + 1):
            ...
            out_hashes[off + local_c] = curr_f if curr_f < curr_r else curr_r
            out_pos[off + local_c] = j
            out_ids[off + local_c] = i
            local_c += 1
